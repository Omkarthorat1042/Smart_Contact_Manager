# Smart Contact Manager

Smart Contact Manager is a web-based application built using **Spring Boot** and **Thymeleaf** to manage and store contact information. This project allows users to create, update, view, and delete contacts in a user-friendly interface. It is designed to provide an intuitive experience for managing personal contacts, with features like user authentication and password management.

## Features

- **User Authentication**: Users can register and log in to access their contact information.
- **Add, Edit, and Delete Contacts**: Easily manage contacts with full CRUD (Create, Read, Update, Delete) functionality.
- **Contact Search**: Search for contacts by name, phone number, or email.
- **Responsive Design**: The application is designed to be mobile-friendly and easy to use on different devices.

## Tech Stack

- **Backend**: Spring Boot, Spring Security
- **Frontend**: Thymeleaf, HTML, CSS, JavaScript
- **Database**: MySQL
- **Tools**: Maven, Git

## Setup and Installation

### Prerequisites

- JDK 8 or higher
- Maven
- MySQL database
- IDE like IntelliJ IDEA or Eclipse

### Steps to Run

1. **Clone the repository**:
   ```bash
   git clone https://github.com/Omkarthorat1042/Smart_Contact_Manager.git
